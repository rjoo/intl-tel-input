...
describe("isPossibleNumber:", function() {
  ...
  describe("init plugin and call public method isPossibleNumber", function() {
    ...
    it("returns true for: valid intl number", function() {
      ...
      expect(input.intlTelInput("isPossibleNumber")).toBeTruthy();
    });

    it("returns false for: invalid intl number", function() {
      input.intlTelInput("setNumber", "+44 123");
      expect(input.intlTelInput("isPossibleNumber")).toBeFalsy();
    });

    it("returns null when utils script is not available", function() {
      delete window.intlTelInputUtils;
      input.intlTelInput("setNumber", "+44 7733 123456");
      expect(input.intlTelInput("isPossibleNumber")).toBeNull();
    });
  ...
  describe("init plugin with nationalMode=true and call public method isPossibleNumber", function() {
    ...
    it("returns false for: correct selected country, invalid number", function() {
      input.intlTelInput("setCountry", "gb");
      input.intlTelInput("setNumber", "123");
      expect(input.intlTelInput("isPossibleNumber")).toBeFalsy();
    });
